...
#define NYI(msg)                            \
  ...
  LogicalResult infer(vector::LoadOp op) {
    ...
    if (rank == 1) {
      ...
      // In vregs layouts, (1, target_shape_[1]) tiling is equivalent to
      // (1, target_shape[1] * n) but we establish the former as canonical.
      const std::array<int64_t, 2> tiling{1, target_shape_[1]};
      // TODO(apaszke): We could generate replicated loads for short values.
      setLayout(op, in_layout,
                VectorLayout(bitwidth, {0, tile_offsets[0]}, tiling,
                             ImplicitDim::kSecondMinor));
    } else {  // rank >= 2
  ...
  LogicalResult infer(vector::StoreOp op) {
    ...
    if (rank == 1) {
      ...
      // (1, target_shape[1] * n) but we establish the former as canonical.
      const std::array<int64_t, 2> tiling{1, target_shape_[1]};
      store_layout = VectorLayout(bitwidth, {0, tile_offsets[0]}, tiling,
                                  ImplicitDim::kSecondMinor);
    } else {  // rank >= 2  // NOLINT(readability-else-after-return)
